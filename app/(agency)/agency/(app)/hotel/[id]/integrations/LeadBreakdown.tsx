    ...
    <section className="rounded-lg border border-line bg-card">
      <div className="border-b border-line p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 text-sm">
          <span className="text-ink-secondary">
            From ads <span className="font-semibold tabular-nums text-ink">{formatNumber(p.fromAds)}</span>
          </span>
          <span className="text-ink-tertiary">
            All leads <span className="tabular-nums">{formatNumber(p.all)}</span>
          </span>
        </div>
        ...
        <p className="mt-2 text-sm text-ink-secondary">
          Booked{" "}
          <span className="font-semibold tabular-nums text-ink">{formatNumber(p.bookedFromAds)}</span>{" "}
          from ads{" "}
          <span className="text-ink-tertiary">
        ...
        <p className="mt-1 text-xs text-ink-disabled">
          Every lead that reached a booking, including any that have since moved to a later bucket.
        </p>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {p.buckets.map((b) => (
              <tr key={b.bucket} className="border-b border-line last:border-0">
                <td className="px-4 py-2 text-ink">{b.bucket}</td>
                <td className="px-4 py-2 text-right tabular-nums text-ink">
                  {b.fromAds === 0 ? <span className="text-ink-disabled">0</span> : formatNumber(b.fromAds)}
                </td>
